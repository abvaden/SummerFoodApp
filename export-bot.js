...
AWS.config.loadFromPath('./aws-lambda-user-settings.json');
...
getBotDefinition(myBotName, myBotVersion, function(err, botDefinition) {
    ...
    if (fs.exists('./lib/bot-fulfillment/bot-setup.json')){
        fs.unlinkSync('./lib/bot-fulfillment/bot-setup.json');
    }
    fs.writeFileSync('./lib/bot-fulfillment/bot-setup.json', JSON.stringify(botDefinition));
	console.log('Bot setup successfully exported');
});