...
function handleShowLocationRequest(showLocationRequest, callback) {
    console.log('Handle showLocationRequest called');
    ...
    getFoodLocationsPromise.then((data) => {
       let response = close(
        ...
        'Fulfilled',
        {
            contentType: 'PlainText', 
            content: 
            'Here is the closest location I found to you ' + 
            data.features[0].attributes.siteName + 
            ' you can contact them at : ' + 
            data.features[0].attributes.contactPhone
        });
...
function handleSelectMealRequest(selectMealRequest, callback) {
    ...
    let reverseGeocodeCallback = (location) => {
        ...
        let summerFoodLocationCallback = (foodResults) => {
            ...
            lexRequest.sessionAttributes.FoodLocationsCount = foodResults.features.length;

            let response = null;
            if (foodResults.features.length > 0) {
                response = close(
                lexRequest.sessionAttributes,
                'Fulfilled',
                {
                    ...
                    content: 
                    'Great! I found ' + 
                    foodResults.features.length  + 
                    ' locations near your search , try saying show me what you found or e-mail the results'
                });
            } else {
                response = close(
                lexRequest.sessionAttributes,
                'Fulfilled',
                {
                    ...
                    ' try another location or try specifying a radius for your search larger than ' + 
                    + selectMealRequest.getRadius() + ' meters'
                });
            }
            callback(response);
        };
