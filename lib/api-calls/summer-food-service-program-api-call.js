...
var makeQuery = function(location, distance) {
    ...
    var deltaLng = distance / (Math.cos(location.lat * (Math.PI / 180.0)) * 111000.0) / 2.0;
...
export function closestLocation(location, features) {
    ...
    for(let i = 0; i < features.length; i++) {
        let distance = distanceToLocation(
            location, 
            {lat: features[i].geometry.y, lng: features[i].geometry.x});
...
export function distanceToLocation(source, location) {
    if(!validateLocation(source) || !validateLocation(location)) {
        throw new Error('Invalid location');
    }

    let deltaLat = source.lat - location.lat;
    let deltaLng = source.lng - location.lng;

    let distance = Math.sqrt(
        Math.pow(deltaLat * 111000.0,2) + 
        Math.pow(deltaLng * (Math.cos(location.lat * (Math.PI / 180.0)) * 111000.0),2));    
...
export function filterByMeals(
    ...
    locations.forEach(function(element) {
        if ((element.attributes === null) || (element.attributes.mealTypesServed === null)) {
